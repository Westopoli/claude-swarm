...
## Where /swarm-spawn fits

**/swarm-spawn is the decomposition step** — it runs after the discovery skill `/swarm` (or after the user has drafted spec/contract/umbrella by hand). Cascade order:

```
[optional] /swarm  → discovery: produces spec + type contract + umbrella (RED), user-confirmed.
                     Skip if the user already has these inputs.
                     ↓
/swarm-spawn       → produces leaf briefs in <briefs_dir>/. STOPS after writing briefs.
...
- a **spec file** under `spec_dir/` — /swarm-spawn does NOT draft specs.
- a **shared type contract** at `type_contract_path` — /swarm-spawn does NOT draft type contracts.
- an **umbrella test** that `umbrella_test_cmd` runs — /swarm-spawn does NOT write umbrella tests; it only confirms RED.
...
Before step 1, lock the scope of this batch. The questions below exist because the most common cascade-failure mode starts with one of these answers being **inferred** by the parent agent instead of stated by the user. (If the user came directly from `/swarm`, several of these answers may already be in chat — re-confirm rather than re-ask.)
...
```markdown
## Assumptions made during /swarm-spawn
...
Every assumption is a candidate for the **parent assumption-sweep** (see end of this file). Do not bury inferences inside step 6 brief prose — log them here where a downstream sweep can find them. (If `/swarm` ran upstream, append to this same file rather than overwriting — `/swarm`'s own assumption log lives at `<spec_dir>/<name>.UNSTATED.md` and is separate.)
...
- Read `type_contract_path`. If missing, stop and recommend running `/swarm` first — `/swarm` is the discovery step that drafts the contract with explicit user-confirmation gates. Do not draft one inside `/swarm-spawn`; that conflates decomposition with discovery.
...
- If the umbrella file doesn't exist, stop and recommend running `/swarm` first — `/swarm` drafts the umbrella with the user. Do not invent a location or write the file from inside `/swarm-spawn`.
...
- Draft the spec, the type contract, or the umbrella test. All three are inputs that must exist before /swarm-spawn runs. Drafting any of them inside this procedure means /swarm-spawn is being used at the wrong step in the cascade — stop and recommend the user run `/swarm` (the discovery step) first.

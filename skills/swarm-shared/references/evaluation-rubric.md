...
The cascade exists because a single agent attempting a large plan loses resolution at the leaf level and silently drifts from the source-of-truth design doc ("the bible," "strategy doc," whichever name the project uses). The skill performs well when it **prevents the four failure modes below at scale**.

### Failure modes the cascade must prevent

1. **Solo-implementation.** Parent agent receives "implement X" and starts editing source files itself instead of decomposing. The catalyzing incident: wave 1 of a real project was built solo in Python when the strategy doc required SQL — recovery cost a full afternoon of revert + redo.
2. **Plan drift from bible.** Spec / decomposition silently disagrees with the strategy doc and the planning agent self-certifies "no conflict." User quote: *"I asked the last chat if its plan conflicted with the strategy doc, it said that it didn't. Clearly it was wrong."*
...
| Skill makes parent diff spec against bible line-by-line and report contradictions | Parent self-certifies "spec matches bible" with no evidence |
...
- Faults seeded **subtly and deep**: overlap on a file whose name is one character different from a sibling; bible-drift in a single brief whose `contract_imports` cite a type that doesn't exist in `type_contract_path`; sizing violation in the budgets, not in obvious prose.
...
- **Evidence quality** — did it cite the right artifact (file path, line, brief id, quote from bible)? Self-certification with no evidence = fail even if verdict is right.

...
.swarm/
  ...
  wave-NN.SWEEP.md              ← parent's aggregate assumption-sweep report; required before first merge of wave
...
- If prior entries for this wave exist (subsequent merge): skip this gate, the wave-sweep already passed at first merge.
...
### 0.7 Wave-snapshot init (first merge of wave only)

If this is the first merge of the wave (same condition as G7 above), compute SHA-256 of **every file in the repo that is not declared in any wave-N brief's `test_files` + `impl_files`**. Write the map to `.swarm/wave-<wave>.snapshot.json`:
...
If `.swarm/wave-<wave>.snapshot.json` already exists, skip this step — it was written at first merge.
...
Leaves may publish questions to `.swarm/questions/leaf-NN-Q<n>.md` instead of inferring silently. The parent answers asynchronously in `.swarm/answers/leaf-NN-Q<n>.md`. This gate enforces that every published question has been resolved before its leaf merges.
...
> ⚠ G3: leaf-NN published question `leaf-NN-Q<n>.md` that is unanswered AND not acknowledged in ASSUMPTIONS. Either answer the question or have the leaf log it as `unanswered: true` before merging.
...
Leaves that need a parent-owned file changed must not edit it (G1 blocks that). Instead, the leaf writes `.swarm/proposals/leaf-NN.md` with a proposed diff and proceeds with its remaining work. This gate enforces that proposals are explicitly resolved before merge.
...
> ⚠ G4: leaf-NN filed an unresolved contract proposal at `.swarm/proposals/leaf-NN.md`. Parent must accept, reject, or supersede before merge. Accepted proposals require the parent to first apply the diff to the parent-owned target file.
...
> ⚠ G4: leaf-NN's proposal is marked accepted but the change was never applied to `<target>`. Apply the change first, then re-run merge.
...
- Load `.swarm/wave-<wave>.snapshot.json`. If absent → **block** (snapshot should have been written at first merge; absence means the wave skipped 0.7).
...
> - ...
>
> Some leaf wrote outside its declared footprint, OR the parent edited a file mid-wave without re-snapshotting. Identify the writer, revert the unauthorised change, or update the wave brief and re-snapshot. Do not merge until the snapshot is clean.

- **Permitted exception:** a path that appears in *this leaf's* `test_files` + `impl_files` is allowed to differ (the leaf is the legitimate owner via staging). The check applies only to paths outside the merging leaf's footprint.

This gate is **detection, not prevention**. The skill cannot revoke Write permission from sub-agents. It can only refuse to merge when a violation is evident. Treat any G5 hit as a leaf-discipline failure to investigate, not a routine warning to dismiss.
...
- Parse the merging leaf's brief for an `escalation_triggers:` frontmatter list. Each entry may have a `detect:` shell command.
...
> ⚠ G6: escalation trigger `<trigger-name>` fired (detect command exited 0) but no `.swarm/escalations/leaf-NN.md` was filed. The brief said this condition required escalation; the leaf either ignored the trigger or buried the decision. Investigate before merge.
...
> - leaf-27: G4 — proposal status is `pending`; parent must resolve before merge
...
When all leaves merge cleanly AND the apex test passes (or is unconfigured), print:

```
Queue complete — N leaves merged. Apex: <PASS | skipped | FAILED>.
...
- Answer a leaf's published question by editing `.swarm/answers/` on the leaf's behalf. Answers are parent decisions; the merge skill only verifies their presence and consistency.
...
- **Sibling-ASSUMPTIONS read** — leaves may *read* (never write) other leaves' `leaf-??.ASSUMPTIONS.md`. Catches drift at leaf-time instead of merge-time. No skill enforcement: it is a brief-boilerplate behavior the leaf executes before logging its own inference.
- **Question ledger** — leaf publishes `.swarm/questions/leaf-NN-Q<n>.md`; parent answers in `.swarm/answers/leaf-NN-Q<n>.md`. **G3 gate** above enforces resolution before merge.
- **Contract proposals** — leaf publishes `.swarm/proposals/leaf-NN.md` instead of editing parent-owned files. **G4 gate** above enforces explicit parent resolution before merge.
